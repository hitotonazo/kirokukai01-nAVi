...
function flowQ2() {
  const steps = [
    {
      prompt:
        'それでは、行方不明者たちの「共通する背景」を探ってみましょう。その背景は、どの資料から読み取れましたか？（複数ある場合は「、」で区切ってください。例：祭事報告書，広報こだま）',
  ...
  const steps = [
    {
      ...
      hints: [
        ...
        '「こだまアート広場」の記事や投稿の中にも、同じ姓の人物がいないか確認してみましょう。そこで、ブログの管理人に関する手がかりが得られるかもしれません。'
      ]
    },
    {
      prompt:
        'では次に、田島里江と深い繋がりを持っていた人物の名前を教えてください。',
      expect: '^\\s*佐久間\\s*圭介\\s*$',
      capture: 'linked',
      praise:
